...
export const BarChart = ({ chartData, xaxis, yaxis, chartTitle, chartColor, setFilters, filters }) => {
    ...
    const onClick = async (event) => {
        ...
        if (clickedPortionData && clickedPortionData.length > 0) {
            console.log("On chart click", clickedPortionData[0])
            const filterKey = lowerCase(xaxis)
            const filterValue = lowerCase(labels[clickedPortionData[0].index])
            console.log("Filterkey and value", filterKey, filterValue)
